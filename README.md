...
```
단위 가격이 높은 제품(비싼 제품)일수록 높은 등급(비싼 제품)의 긍정 비율이 낮아지는 경향이 있다.
```
![g단위 긍정비율별 트렌드](./images/posit_trend_gram.png)
```
긍정문의 비율이 높은 제품일수록 높은 등급(비싼 제품)의 긍정 비율이 낮아지는 경향이 있다.
```
...
```
단위 가격이 높은 제품(비싼 제품)일 수록 높은 등급(비싼 제품)의 긍정 비율이 높아지는 경향이 있다.

```
![ml단위 긍정비율별 트렌드](./images/posit_trend_liter.png)
```
긍정문의 비율이 낮은 제품일수록 높은 등급(비싼 제품)의 긍정 비율이 높아지는 경향이 있다.
